...
tbl_arranged %>%
  ...
  row_spec(rows_fallback, background = "#ffd7b4ff") %>%
  add_header_above(c(
    " "                          = 4,
...
p1 <- ggplot(plot_data_h1, aes(x = estimate, y = Manual_class, color = sig)) +
  ...
  geom_point(size = 2) +
  scale_color_manual(
    values = c("Significant" = "#ff7f0eff", "Non-significant" = "#bcbcbc")
  ) +
  ...
  annotate("text", x =  0.7, y = 0.3, label = "Lower stability in C →",
...
plot_data_h1 %>%
  ...
  column_spec(6, color = ifelse(plot_data_h1$sig == "Significant", "#ff7f0eff", "#bcbcbc"))
```
...
p2 <- ggplot(h2_data, aes(x = est_man, y = est_classification)) +
  ...
  ggrepel::geom_text_repel(aes(label = Taxon), size = 2,
                            vjust = 0.3, hjust = 0.5) +
  labs(
    x = "Estimated effect from manual classification data",
    y = "Estimated effect from model classification data"
  )
