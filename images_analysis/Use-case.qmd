...
Note: `r2_nakagawa()` cannot decompose the AR1 random-effect variance in `glmmTMB`
models and would silently return a biased (fixed-effects-only) value. R² is therefore
not reported here. Model fit is instead assessed through DHARMa diagnostics and
the congruence analysis in Section 4.
...
#|           DHARMa diagnostics. Taxa are ordered from the largest to the smallest
#|           land-use effect (Position C vs. A). Effect sizes are on the log-link
#|           scale: positive values indicate lower stability (higher CV) in the
#|           cultivated zone. Model Status: 'Robust' = all DHARMa checks passed;
#|           'Fallback RI' = AR1 model did not converge and a random-intercept model
#|           was used instead; 'Non-Convergence' = neither model converged (estimates
#|           should be interpreted with caution); 'Discrete/Non-Uniform' = expected
...
rows_non_conv <- which(tbl_arranged$Status == "Non-Convergence")
...
tiff("../images_analysis/output/Fig.H1_population_stability_across_position.tiff",
...
tiff("../images_analysis/output/Fig.H2_technological_congruence.tiff",
     width = 1000, height = 1000, res = 300)
p2
invisible(dev.off())
```
